...
// Ambil data user role admin
$result = $koneksi->query("SELECT * FROM users WHERE role='admin'");
?>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-fluid py-4">
                <h1 class="mb-4 text-primary"><i class="fas fa-users-cog"></i> Kelola Data Admin</h1>
                <?php if (isset($_GET['msg'])): ?>
                    <div id="alert-msg"
                        class="alert <?= $_GET['msg'] == 'hapus_sukses' ? 'alert-success' : 'alert-danger' ?>">
                        <?= $_GET['msg'] == 'hapus_sukses' ? 'Admin berhasil dihapus!' : 'Gagal menghapus admin!' ?>
                    </div>
                <?php endif; ?>

                <!-- Tombol Tambah Admin -->
                <button class="btn btn-primary mb-3" type="button" id="btnTambahAdmin">
                    + Tambah Admin
                </button>

                <!-- Form Tambah Admin -->
                <?php
                // Tampilkan form langsung jika ada error
                $form_display = !empty($errors) ? 'block' : 'none';
                ?>
                <div class="card mb-4" id="formTambahAdmin" style="display:<?= $form_display ?>;">
                    ...
                    <tbody>
                        ...
                        while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?= $row['role']; ?></td>
                                <td>
                                    <a href="admin_edit.php?id=<?= $row['id_user']; ?>"
                                        class="btn btn-warning btn-sm">Edit</a>
                                    <a href="admin_delete.php?id=<?= $row['id_user']; ?>" class="btn btn-danger btn-sm"
